<template>
  <el-card
    :class="this.anime === undefined ? 'hvr-grow-shadow' : ''"
    :style="this.anime === undefined ? '' : 'height:490px'"
  >
    <el-tag class="rentTagStatus" :type="rentType">
      {{ rentStatus }}
    </el-tag>
    ...
    <div class="brief-msg">
      ...
      <!--右边收藏按钮-->
      <div class="right">
        <el-tag :type="tagType" @click.stop="favorite">{{
          favoriteOrTag
        }}</el-tag>
      </div>
    </div>
    ...
    <el-dialog title="房屋配置" :visible.sync="dialogVisible" width="50%">
      <el-tag
        v-for="item in tags"
        :key="item.hTid"
        type="success"
...
export default {
  ...
  methods: {
    favorite() {
      ...
      if (this.favoriteAndTag > 0) {
        this.$http
          .get("getHouseAllTag", { params: { hid: this.d.hid } })
          .then((res) => {
  ...
  computed: {
    ...
    // 计算是否已经出租
    rentStatus() {
      return this.d.houseStatus > 0
        ? this.d.houseStatus === 1
          ? "出租中"
          : "已租赁"
        : "已出租";
    },
